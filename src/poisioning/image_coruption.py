...
def plasma_fractal(
    ...
    *,
    ...
    def wibblemean(array, *, key: PRNGKeyArray):
        denom = 4 + wibble * jax.random.uniform(
            key=key,
            shape=array.shape,
            minval=-wibble,
            maxval=wibble,
    ...
    return maparray / jnp.where(max_val < 1e-8, 1.0, max_val)
...
def rgb2hsv(x: Array) -> Array:
    # x = x / 255.0
    r, g, b = (
        jnp.expand_dims(x[0, ...], axis=0),
        jnp.expand_dims(x[1, ...], axis=0),
        jnp.expand_dims(x[2, ...], axis=0),
    )
...
def gaussian_noise(x: Array, severity: int = 1, *, key: PRNGKeyArray) -> Array:
    ...
    x = x / 255.0
    return (
        jnp.clip(x + jax.random.normal(key, x.shape, dtype=jnp.float32) * c, 0, 1) * 255
    ).astype(jnp.uint8)
...
def shot_noise(x: Array, severity: int = 1, *, key: PRNGKeyArray) -> Array:
    c = [0.04, 0.06, 0.08, 0.09, 0.10][severity - 1]
    # x = x * 255.0
    return (jnp.clip(x + jax.random.poisson(key, lam=(x * c)) / c, 0, 255)).astype(
        jnp.uint8
    )
...
def impulse_noise(x: Array, severity: int = 1, *, key: PRNGKeyArray):
    ...
    x = jnp.where(salt, 1.0, x)
    x = jnp.where(pepper, 0.0, x)
...
def speckle_noise(x: Array, severity: int = 1, *, key: PRNGKeyArray) -> Array:
    ...
    return (
        jnp.clip(
            x + x * jax.random.normal(key, shape=x.shape, dtype=jnp.float32) * c, 0, 1
        )
        * 255
    ).astype(jnp.uint8)
